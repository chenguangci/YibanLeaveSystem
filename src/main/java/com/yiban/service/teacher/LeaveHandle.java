...
public class LeaveHandle {
     ...
     */
    @Transactional
    public Result agreeLeave(long id) throws SendException, RequestInfoException, ReSetTokenException, SystemRunTimeException {
        //生成验证码,存入数据库
        if (contentMapper.updateLeave(id, 1, getCode(id)) > 0) {
            //发送信息给请假人
     ...
     */
    @Transactional
    public Result disagree(long id) throws SendException, RequestInfoException, ReSetTokenException, SystemRunTimeException {
        //更新数据库
        if (contentMapper.updateLeaveWithoutCode(id, -1) > 0) {
            //发送信息给请假人
